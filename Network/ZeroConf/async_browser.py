...
def async_on_service_state_change(
    ...
    """
    async_on_service_state_change Callback for service state change

    Args:
        zeroconf (Zeroconf): ZeroConf object to be used
        service_type (str): Type of the service
        name (str): Name of the service
        state_change (ServiceStateChange): new state
    """
...
async def async_display_service_info(zeroconf: Zeroconf, service_type: str, name: str) -> None:
    """
    async_display_service_info display the service information
    ...
    if info:
        addresses = [
            "%s:%d" % (addr, cast(int, info.port)) for addr in info.parsed_scoped_addresses()
        ]
        print("  Name: %s" % name)
...
class AsyncRunner:
    ...
    async def async_run(self, ip_version) -> None:
        """
        async_run run the discovery in the background
        """
    ...
    async def async_close(self) -> None:
        """
        async_close close the connection asynchronously
        """
...
def main():
    ...
    try:
        loop.run_until_complete(runner.async_run(ip_version))
