...
from fault import UPnPError
from util import get_dict_val_case_insensitive
from serialization.scpd import deserialize_scpd_get_response
from serialization.scpd import serialize_scpd_get
from serialization.soap import serialize_soap_post, deserialize_soap_post_response
...
class SCPDHTTPClientProtocol(Protocol):
    ...
    def __init__(
        self,
        message: bytes,
        finished: "asyncio.Future[typing.Tuple[bytes, bytes, int, bytes]]",
        soap_method: typing.Optional[str] = None,
        soap_service_id: typing.Optional[str] = None,
    ) -> None:
                ...
                if self._got_headers and self._has_content_length:
                    ...
                    if self._content_length == len(body):
                        self.finished.set_result(
                            (self.response_buff, body, self._response_code, self._response_msg)
                        )
...
async def scpd_get(
...
) -> typing.Tuple[typing.Dict[str, typing.Any], bytes, typing.Optional[Exception]]:
    ...
    proto_factory: typing.Callable[[], SCPDHTTPClientProtocol] = lambda: SCPDHTTPClientProtocol(
        packet, finished
    )
    try:
        connect_tup: typing.Tuple[
            asyncio.BaseTransport, asyncio.BaseProtocol
        ] = await loop.create_connection(proto_factory, address, port)
    ...
    error = None
    wait_task: typing.Awaitable[typing.Tuple[bytes, bytes, int, bytes]] = asyncio.wait_for(
        protocol.finished, 1.0, loop=loop
    )
    body = b""
    raw_response = b""
    try:
...
async def scpd_post(
    ...
    **kwargs: typing.Dict[str, typing.Any],
    ...
    try:
        return (
            deserialize_soap_post_response(body, method, service_id.decode()),
            raw_response,
            None,
        )
    except Exception as err:
